...
use crate::{get_processing_time, tumbling::TumblingWindow};

pub trait TumblingProcessingTimeWindow<K, V, T, VO> {
    /// TUmbling processing time window which passes a reference of all values part of the window
    ...
    /// use malstrom_windows::processing::TumblingProcessingTimeWindow;
    ///
    ...
    ///             .tumbling_processing_time_window("join-two", 200, 0, |x| {
    ...
    ///      .execute()
    ///      .unwrap();
    /// let expected: Vec<_> = vec![1, 5];
    /// let out: Vec<_> = sink.into_iter().map(|x| x.value).collect();
    /// assert_eq!(out, expected);
    /// ```
    fn tumbling_processing_time_window(
        self,
        name: &str,
        size: T,
        zero_alignment: T,
...
impl<K, V, T, VO> TumblingProcessingTimeWindow<K, V, u128, VO> for StreamBuilder<K, V, T>
where
    K: Key + Distributable,
        ...
        + Copy,
{
    fn tumbling_processing_time_window(
        ...
        stream.tumbling_window(
            &format!("{name}-processing-tumbling-window"),
            size,
            zero_alignment,
            aggregator,
        )
...
mod tests {
    ...
    use crate::processing::TumblingProcessingTimeWindow;
