...
use crate::channels::selective_broadcast::{full_broadcast, link, Sender};
...
use crate::{Data, Key, MaybeKey, Message, NoData, NoKey, WorkerId};
...
fn pass_messages<K: Clone, V: Clone, T: Clone, P>(input: &mut Receiver<K, V, T, P>, output:  &mut Sender<K, V, T, P>) -> () {
    match input.recv() {
...
fn build_leader_controller_logic<K: MaybeKey, V: Data, T: MaybeTime, P: PersistenceBackend>(
    ...
    let mut in_progress_snapshot: Option<(IndexMap<WorkerId, bool>, SnapshotVersion, Barrier<P>)> = None;

    move |input: &mut Receiver<K, V, T, P>, output:  &mut Sender<K, V, T, P>, ctx: &mut OperatorContext| {
        ...
        if !in_progress_snapshot.is_none() && timer() {
            ...
            let _ = in_progress_snapshot.insert(
                ...
                (ctx.communication.get_peers().into_iter().chain(std::iter::once(&0usize)).map(|x| (x.to_owned(), false)).collect(),
                snapshot_version, barrier.clone())
            );
        ...
        for msg in ctx.communication.recv_all() {
            match msg {
                ComsMessage::StartSnapshot(_i) => {
                    unreachable!("Lead node received instruction which can only be given by lead node")
                }
    ...
    move |input: &mut Receiver<K, V, T, P>, output:  &mut Sender<K, V, T, P>, ctx: &mut OperatorContext| {
        ...
        for msg in ctx.communication.recv_all() {
            match msg {
                ...
                ComsMessage::LoadSnapshot(i) => {
                    output.send(Message::Load(Load::new(P::new_for_version(ctx.worker_id, &i))))
                }
