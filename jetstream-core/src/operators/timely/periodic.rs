...
impl<K, V, T, P> PeriodicEpochs<K, V, T, P> for NeedsEpochs<K, V, T, P> where K: MaybeKey, V: MaybeData, T: Timestamp, P: PersistenceBackend {
    fn generate_periodic_epochs(
        self,
        worker: &mut Worker<P>,
        // previously issued epoch and sys time elapsed since last epoch
        gen: impl FnMut(&Option<(&T, Duration)>) -> Option<T> + 'static,
    ) -> (JetStreamBuilder<K, V, T, P>, JetStreamBuilder<K, V, NoTime, P>) {
        self.0.generate_periodic_epochs(worker, gen)
    }
}

impl<K, V, T, P> PeriodicEpochs<K, V, T, P> for JetStreamBuilder<K, V, T, P> where K: MaybeKey, V: MaybeData, T: Timestamp, P: PersistenceBackend {
    fn generate_periodic_epochs(
        self,
        worker: &mut Worker<P>,
        // previously issued epoch and sys time elapsed since last epoch
        ...
        let operator = OperatorBuilder::built_by(move |build_context| {
            let mut state: Option<(T, Instant)> = build_context.load_state().unwrap_or_default();
            move |input, output, ctx| {
                // check if the user function want's to create an epoch
                let now = Instant::now();
                let elapsed = state.as_ref().map(|(t, i)| (t, now.duration_since(*i)));
