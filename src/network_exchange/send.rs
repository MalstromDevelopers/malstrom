...
use std::{marker::PhantomData, net::SocketAddr, time::{Duration, Instant}};
use crate::{
...
use super::{grpc::ExchangeServer, ExchangeData, Remote, CONFIG, api::exchange_message::ExchangeContent};
    ...
    pub fn schedule<P: PersistenceBackend>(
        ...
        if now.duration_since(self.last_heartbeat) > HEARTBEAT_INTERVAL {
            ...
            for i in 0..self.remote_count {
                self.server.send(ExchangeContent::Frontier(frontier.get_actual().into()), i);
            }
        ...
        match input.recv() {
            ...
            Some(BarrierData::Barrier(mut b)) => {
                ...
                for i in 0..self.remote_count
                {
                    self.server.send(ExchangeContent::Barrier(super::api::Barrier{}), i);
                }
            ...
            Some(BarrierData::Data(d)) => {
                ...
                if idx == 0 {
                    ...
                    self.server.send(ExchangeContent::Data(encoded), idx - 1).expect("Out of range partition");
                }
            }
            None => ()
        }
