...
use super::{grpc::ExchangeClient, ExchangeData, Remote, CONFIG};
    ...
    P: PersistenceBackend,
{
    ...
    pub fn schedule<O: ExchangeData>(
        &mut self,
        input: &mut Receiver<NoData, P>,
        ...
        if self.remote_barrier.is_none() {
            for msg in self.connection.recv_all() {
                match msg {
                    ExchangeContent::Barrier(_) => {
                        self.remote_barrier = Some(());
                        break;
                    }
        ...
        (self.local_barrier, self.remote_barrier) =
            match (self.local_barrier.take(), self.remote_barrier.take()) {
                (Some(mut b), Some(_)) => {
